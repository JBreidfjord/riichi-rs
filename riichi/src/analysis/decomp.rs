...
use riichi_decomp_table::{
    CTable, CompleteGrouping, make_c_table, c_entry_iter,
    WTable, WaitingPattern, WaitingKind, make_w_table, w_entry_iter,
};
...
impl FullHandWaitingPattern {
    pub fn groups(&self) -> impl Iterator<Item=HandGroup> {
        self.raw_groups.to_le_bytes().into_iter()
...
impl Decomposer<'_> {
    ...
    pub fn iter<'a>(&'a self)
        ...
        let suit_x = self.c_for_suit.iter()
            .map(|v| v.len()).enumerate()
            .fold(4, |ii, (i, len)|
                if len != 0 { ii } else if ii == 4 { i as u8 } else { 5 },
            );
        ...
        [
            ...
            .flat_map(move |(suit_w, suits_c)| {
                let chain =new_partial_iter(
                    self.c_table, self.w_table, suit_w, self.keys[suit_w as usize]);

                let chain =extend_partial_iter(
                    chain, suits_c[0], &self.c_for_suit[suits_c[0] as usize]);
                let chain =extend_partial_iter(
                    chain, suits_c[1], &self.c_for_suit[suits_c[1] as usize]);
                let chain =extend_partial_iter(
                    chain, suits_c[2], &self.c_for_suit[suits_c[2] as usize]);

                chain.flat_map(move |partial|
                    PartialDecomp::to_full_hand(partial, suit_w))
            })
    }
}
...
impl PartialDecomp {
    ...
    fn try_extend(&self, suit: u8, c: &CompleteGrouping) -> Option<PartialDecomp> {
        if (self.has_pair_or_tanki && c.pair().is_some()) ||
            (suit == 3 && c.has_shuntsu()) {
            return None;
        }
        Some(PartialDecomp {
            w: self.w,
    ...
    fn to_full_hand(self, suit: u8) -> impl Iterator<Item=FullHandWaitingPattern> {
        [self.w.waiting_pos_low(), self.w.waiting_pos_high()].into_iter()
            .filter_map(move |pos|
                pos.and_then(|pos|
                    Tile::from_encoding(pos + suit * 9)))
            .map(move |waiting_tile| {
                FullHandWaitingPattern {
...
fn new_partial_iter<'a>(c_table: &'a CTable, w_table: &'a WTable, suit: u8, key: u32)
                        -> impl Iterator<Item=PartialDecomp> + 'a {
    w_iter(w_table, key).flat_map(move |w|
        c_iter(c_table, w.complete_key).filter_map(move |c|
            PartialDecomp::new(w).try_extend(suit, &c)))
}

fn extend_partial_iter<'a>(partial_iter: impl Iterator<Item=PartialDecomp> + 'a,
                           suit: u8, c: &'a [CompleteGrouping])
                           -> impl Iterator<Item=PartialDecomp> + 'a {
    partial_iter.flat_map(move |partial| {
        c.iter().filter_map(move |c| partial.try_extend(suit, c))
    })
}

fn extend_groups(full_groups: u32, suit: u8, c: &CompleteGrouping) -> u32 {
    c.groups().fold(full_groups,
                    |gs, g| (gs << 8) | ((g | (suit << 4)) as u32))
}
...
fn c_iter<'a>(c_table: &'a CTable, key: u32) -> impl Iterator<Item=CompleteGrouping> + 'a {
    c_table.get(&key).into_iter().flat_map(move |&value| c_entry_iter(key, value))
}

fn w_iter<'a>(w_table: &'a WTable, key: u32) -> impl Iterator<Item=WaitingPattern> + 'a {
    w_table.get(&key).into_iter().flat_map(move |&value| w_entry_iter(key, value))
}
...
mod tests {
    ...
    fn print_decomp(keys: [u32; 4]) {
        println!("=== {:09o} {:09o} {:09o} {:07o} ===", keys[0], keys[1], keys[2], keys[3]);
